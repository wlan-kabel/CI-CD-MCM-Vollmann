...
func TestCreateProductInvalidJSON(t *testing.T) {
	...
	body := `{"name":"Widget","price":invalid}`
	req := httptest.NewRequest("POST", "/products", strings.NewReader(body))
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestCreateProductInvalidEmptyName(t *testing.T) {
	r, _ := setupRouter()
...
func TestCreateProductInvalidNegativePrice(t *testing.T) {
	r, _ := setupRouter()

	body := `{"name":"Widget","price":-5.0}`
	req := httptest.NewRequest("POST", "/products", strings.NewReader(body))
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
